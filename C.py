...
\x1b[0;97m-----------------------------------------------"""
boy = ['Rahul Kumar', 'Lokesh Kumar', 'Sahil Kumar', 'Ram Kumar', '', 'Rajesh Sharma', 'Raj Sharma, 'Nadan Ldka', 'Kalu Baba', 'Sunil Sharma', 'Safdar Malik', 'Nazir khan', 'Saleem Malik', 'Abdullah Malik', 'Naseer Jutt', 'Zain Malik', 'Arhan khan', 'Avi Raj', 'Suraj Raj', 'Vishal Kumar', 'Nitesh Kumar']
...
class create:
    ...
    'viewport-width': '980',}
        ...
        for x in range(lim):
            self.loop += 1
            sys.stdout.write(f'\r{OO}[HONEY-XD] {OO}{self.loop}/{str(lim)} OK: {len(ok)}{OO} = CP: {len(cp)}{OO} '),
            sys.stdout.flush()
                        ...
                        for x in BeautifulSoup(po.text, 'html.parser').find_all('form', {'method': 'post'}):
                            if 'confirmation_event_location=cliff' in str(x):
                                ...
                                if 'checkpoint' in rex.url:
                                    ...
                                    open('/sdcard/HONEY-OK.txt','a').write(+cok['c_user']+'|'+passw+'\n')
